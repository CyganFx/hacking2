...
func main() {
	...
	var tcp layers.TCP
	decodedLayers := make([]gopacket.LayerType, 10)

	parser := gopacket.NewDecodingLayerParser(
		layers.LayerTypeEthernet, &eth, &ipv4, &tcp)
	...
	for {
		...
		for _, layerType := range decodedLayers {
			...
			case layers.LayerTypeTCP:
				...
				assCtx := assemblerSimpleContext(ci)
				stats.totalsz += len(tcp.Payload)
				assembler.AssembleWithContext(transportFlow, &tcp, &assCtx)
			default:
				continue
			}
		}
